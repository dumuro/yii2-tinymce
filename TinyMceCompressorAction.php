...
class TinyMceCompressorAction extends Action
{
    ...
    public function handleRequest()
    {
        ...
        for ($i = 0; $i < count($allFiles); $i++) {
            $file = substr($allFiles[$i], 0,
                1) === '/' ? Yii::getAlias('@webroot' . $allFiles[$i]) : $tinymceDir . "/" . $allFiles[$i];
        ...
        $buffer .= 'tinymce.each("' . implode(',',
                $files) . '".split(","),function(f){tinymce.ScriptLoader.markDone(tinyMCE.baseURL+"/"+f+".js");});';
